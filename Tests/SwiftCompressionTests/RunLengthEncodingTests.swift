...
struct RunLengthEncodingTests {
    @Test func compressRLE() {
        ...
        var compressed = RunLengthEncoding(minRun: 3, alwaysIncludeRunCount: true).compress(data: data, configuration: .init())
        var expected_result:[UInt8] = [196, 65, 196, 66, 196, 67]
        #expect(compressed == expected_result)

        compressed = RunLengthEncoding(minRun: 5, alwaysIncludeRunCount: true).compress(data: data, configuration: .init())
        #expect(compressed == expected_result)

        compressed = RunLengthEncoding(minRun: 6, alwaysIncludeRunCount: true).compress(data: data, configuration: .init())
        #expect(compressed == expected_result)

        compressed = RunLengthEncoding(minRun: 6, alwaysIncludeRunCount: false).compress(data: data, configuration: .init())
        expected_result = data
        #expect(compressed == expected_result)

        data = .init(String(repeating: "A", count: 66).utf8)
        compressed = RunLengthEncoding(minRun: 3, alwaysIncludeRunCount: true).compress(data: data, configuration: .init())
        expected_result = [255, 65, 193, 65]
        #expect(compressed == expected_result)

        data = [190, 191, 192]
        compressed = RunLengthEncoding(minRun: 3, alwaysIncludeRunCount: false).compress(data: data, configuration: .init())
...
extension RunLengthEncodingTests {
    @Test func decompressRLE() throws {
        ...
        let compressed = RunLengthEncoding(minRun: 3, alwaysIncludeRunCount: true).compress(data: data, configuration: .init())
